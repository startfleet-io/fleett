var tripetto = TripettoServices.init({
    token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjoiQWNaSHVwTDlrWjNzR0ZlQXVvZWRvdGV3ZE9udzRJb3BEMXVJNUdWbFRiYz0iLCJkZWZpbml0aW9uIjoibnJvaFk1bXhCVU1US0lUTnQwenJCM01iVm14N3Z5aXhaRWpSb0pOaE15MD0iLCJ0eXBlIjoiY29sbGVjdCJ9.euuacoZnqukVKJfZveGbsmW3fFEM_iJtMIeEZFT6Ic8',
});
...
TripettoClassic.run({
    ...
    onSubmit: function (instance) {
        ...
        fields.forEach(item => {
            ...
            if (item.value !== '' && item.value !== undefined && item.name.indexOf(' ') <= 0 && (item.value != false || item.value != 'false'))
        ...
        if (form_data['7a_responsibleParty'] && form_data['7a_responsibleParty'] !== '') {
...
function init() {
    ...
    if (coldStop == '' || !coldStop || coldStop != 'yes' || savedOrderId != orderID || savedState != pstate || savedStructure != pstructure) {
        ...
        $.ajax({
            ...
            success: function (response) {
                const { company_name, company_state, company_structure, company_suffix, purchase_order_id, full_name, email, phone } = response;
