...
var suffix = {
    ...
    corporation: ['Corporation', 'Incorporated', 'Inc.', 'Corp.'],
};
var codes = [];
var choosenProducts = [];
var x, y, z;
...
function stateValidation(divStep) {
    let stateElements = divStep.find('div.wrapper-state').find('input:checked');
    ...
    if (!stateElements.length) {
        ...
        divStep.find('div.wrapper-state').next('div.invalid-insert').remove();

        divStep.find('div.wrapper-state').after(msg);
...
function structureValidation(divStep) {
    let structureElements = divStep.find('div.wrapper-structure').find('input:checked');

    if (!structureElements.length) {
        let msg = formErrorMsg;
        msg = msg.replace('###', 'Please choose structure!');

        divStep.find('div.wrapper-structure').next('div.invalid-insert').remove();

        divStep.find('div.wrapper-structure').after(msg);
...
function planValidation(divStep) {
    ...
    if (!isMobile()) {
        planElements = divStep.find('div.custom-grid-pricing-step-2').find('input:checked');
    } else {
        planElements = divStep.find('div.slider-2').find('input:checked');
    }

    if (!planElements.length) {
        let msg = formErrorMsg.replace('###', 'Please choose plan!');
        divStep.find('div.custom-grid-pricing-step-2').next('div.invalid-insert').remove();

        divStep.find('div.custom-grid-pricing-step-2').after(msg);
...
$(document).on('click', 'div.state-list-details-wrap', function () {
    ...
    let checkboxes = divStep.find('div.wrapper-state').find("input[type='radio']");
...
$('div.wrapper-structure .checkbox-wrap').on('click', function () {
    ...
    let checkboxes = divStep.find('div.wrapper-structure').find("input[type='checkbox']");
...
$('div.custom-grid-pricing-step-2 .checkbox-wrap').on('click', function () {
    ...
    let checkboxes = divStep.find('div.custom-grid-pricing-step-2').find("input[type='checkbox']");
...
$('div.slider-2 .checkbox-wrap').on('click', function () {
    ...
    let checkboxes = divStep.find('div.slider-2').find("input[type='checkbox']");
...
function callStateFee(data) {
    ...
    let found = dt
        ...
        .find(function (elm) {
            return elm.structure.toLowerCase() == structure && elm.state.toLowerCase() == state;
        });
...
$('div.wrapper-option-member-step-3 label.w-checkbox').click(function () {
    ...
    let checkboxes = divStep.find('div.wrapper-option-member-step-3').find('input:checked');
...
async function finalSubmission() {
    ...
    if (step == 4) {
        ...
        if (choosenProducts.length > 0) {
            ...
            if (selectedItems.length) form_data.addons = selectedItems;
        }
...
function callEncharge(form_data) {
    const { phone, email, full_name, plan, company_name, company_members, company_state } = form_data;
...
function setProductPrices(plan) {
    ...
    choosenProducts.forEach(cp => {
        ...
        let is_included =
            cp[`included_in_${plan}`] === 'true' ? true : cp[`included_in_${plan}`] === 'false' ? false : Boolean(cp[`included_in_${plan}`]);
...
function myinit() {
    ...
    let stateCheckboxes = divStep.find('#state-list-main-wrap').find("input[type='radio']");

    let structureCheckboxes = divStep.find('div.wrapper-structure').find("input[type='checkbox']");
    ...
    if (companyCity && companyType) {
        let found = dt
            .filter(t => t.state !== null)
            .find(function (elm) {
                return elm.structure.toLowerCase() == companyType.toLowerCase() && elm.state.toLowerCase() == companyCity.toLowerCase();
            });
        if (found) {
            let data = JSON.parse(localStorage.getItem(dataName));
            data.stateId = found.id;
    ...
    let planCheckboxes = planScreen.find('div.custom-grid-pricing-step-2').find("input[type='checkbox']");
...
var updateInputValue = function (event) {
    ...
    let data = localStorage.getItem(dataName) ? JSON.parse(localStorage.getItem(dataName)) : {};
...
function onCheckout(step) {
    ...
    const { planId, stateId, refcodes, cEmail, cName, cPhone, country_c, country_r } = data;
...
function myFunction() {
    ...
    if (foundCount <= 0) {
        ul.append(`<div class="last-row-wrap" id="last-row">
         <p>If you are interested in a state that is not found on our list,</p>
         <p>please talk to us on chat.</p>     
         </div>`);
    }
}
