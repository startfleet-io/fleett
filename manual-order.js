$(document).ready(function () {
    ...
    function setRequiredFields() {
        ...
        $('#customer_citizenship').empty().append(`<option value=''>Choose Country</option>`);

        $('#customer_residentialship-2').empty().append(`<option value='' >Choose Country</option>`);

        countries.forEach(c => {
            let selected = c === 'United States' ? 'selected' : '';
            $('#customer_citizenship').append(`<option value='${c}'>${c}</option>`);

            $('#customer_residentialship-2').append(`<option value='${c}'>${c}</option>`);
        });
    }
    setRequiredFields();
            ...
            success: function (res) {
                ...
                if (response.result.url) {
                    Swal.fire({
                        ...
                        html: `Order Number Is: <b>${response.result.metadata.purchase_order_id}</b> <br />You can use this link to pay ==> <a href="${response.result.url}" target="_blank">Pay Here</a> `,
                    }).then(result => {
